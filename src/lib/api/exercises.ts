 ...
 */
export async function hideExercise(pb: PocketBase, user: AuthRecord | null, exerciseId: string) {
 ...
 */
export async function unhideExercise(pb: PocketBase, user: AuthRecord | null, exerciseId: string) {
...
export async function getMuscleGroups(
	...
	// 1) fetch all exercises for the user
	const exercises = await getExercises(pb, user, showHidden);

	// 2) gather all muscle groups
