...
- `lib` contains your library code (utilities and components), which can be imported via the [``]() alias, or packaged up for distribution using [`svelte-package`](packaging)
  - `server` contains your server-only library code. It can be imported by using the [`/server`](server-only-modules) alias. SvelteKit will prevent you from importing these in client code.
...
URLs are represented by the [`URL`](https://developer.mozilla.org/en-US/docs/Web/API/URL) interface, which includes useful properties like `origin` and `pathname` (and, in the browser, `hash`). This interface shows up in various places — `event.url` in [hooks](hooks) and [server routes](routing#server), [`page.url`](-state) in [pages](routing#page), `from` and `to` in [`beforeNavigate` and `afterNavigate`](-navigation) and so on.
...
If your `load` function can only run on the server — for example, if it needs to fetch data from a database or you need to access private [environment variables](-static-private) like API keys — then you can rename `+page.js` to `+page.server.js` and change the `PageLoad` type to `PageServerLoad`.
...
> [!LEGACY]
> `/state` was added in SvelteKit 2.12. If you're using an earlier version or are using Svelte 4, use `/stores` instead.
...
Throughout the examples above, we've been importing types from a `.d.ts` file. This is a file SvelteKit creates for you in a hidden directory if you're using TypeScript (or JavaScript with JSDoc type annotations) to give you type safety when working with your root files.

For example, annotating `let { data } = ()` with `PageProps` (or `LayoutProps`, for a `+layout.svelte` file) tells TypeScript that the type of `data` is whatever was returned from `load`:
...
> ```js
> /// file: +page.svelte
> /** @type {{ data: import('./').PageData, form: import('./').ActionData }} */
> let { data, form } = ();
> ```
>
> Or, for a layout:
...
> ```js
> /// file: +layout.svelte
> /** @type {{ data: import('./').LayoutData, children: Snippet }} */
> let { data, children } = ();
> ```
...
You can read more about omitting `` in our [blog post](/blog/zero-config-type-safety) about it.
...
> ```js
> /// file: +page.svelte
> /** @type {{ data: import('./').PageData }} */
> let { data } = ();
> ```
>
> In Svelte 4, you'd use `export let data` instead.

Thanks to the generated `` module, we get full type safety.
...
/// file: src/routes/blog/[slug]/+page.server.js
// @filename: ambient.d.ts
declare module '/server/database' {
...
+++	// we can access `data.posts` because it's returned from
  ...
  let index = (data.posts.findIndex(post => post.slug === page.params.slug));
...
> [!LEGACY]
> `/state` was added in SvelteKit 2.12. If you're using an earlier version or are using Svelte 4, use `/stores` instead.
> It provides a `page` store with the same interface that you can subscribe to, e.g. `.data.title`.
...
In the browser, you can also navigate programmatically outside of a `load` function using [`goto`](-navigation#goto) from [`.navigation`](-navigation).
...
You can also rerun `load` functions that apply to the current page using [`invalidate(url)`](-navigation#invalidate), which reruns all `load` functions that depend on `url`, and [`invalidateAll()`](-navigation#invalidateAll), which reruns every `load` function. Server load functions will never automatically depend on a fetched `url` to avoid leaking secrets to the client.
...
<script>
  import { invalidate, invalidateAll } from '/navigation';
...
- It declared a dependency on a specific URL via [`fetch`](#Making-fetch-requests) (universal load only) or [`depends`](@sveltejs-kit#LoadEvent), and that URL was marked invalid with [`invalidate(url)`](-navigation#invalidate)
- All active `load` functions were forcibly rerun with [`invalidateAll()`](-navigation#invalidateAll)

`params` and `url` can change in response to a `<a href="..">` link click, a [`<form>` interaction](form-actions#GET-vs-POST), a [`goto`](-navigation#goto) invocation, or a [`redirect`](@sveltejs-kit#redirect).

Note that rerunning a `load` function will update the `data` prop inside the corresponding `+layout.svelte` or `+page.svelte`; it does _not_ cause the component to be recreated. As a result, internal state is preserved. If this isn't what you want, you can reset whatever you need to reset inside an [`afterNavigate`](-navigation#afterNavigate) callback, and/or wrap your component in a [`{#key ...}`](../svelte/key) block.
...
/// file: src/routes/login/+page.svelte
<script>
  import { enhance, +++applyAction+++ } from '/forms';
...
<!--- file: src/routes/login/+page.svelte --->
<script>
  import { invalidateAll, goto } from '/navigation';
  import { applyAction, deserialize } from '/forms';
...
Note that you need to `deserialize` the response before processing it further using the corresponding method from `/forms`. `JSON.parse()` isn't enough because form actions - like `load` functions - also support returning `Date` or `BigInt` objects.
...
While prerendering, the value of `building` imported from [`/environment`](-environment) will be `true`.
...
You might wonder how we're able to use `page.data` and other [app state](-state) (or [app stores](-stores)) if we can't use global state. The answer is that app state and app stores on the server use Svelte's [context API](/tutorial/svelte/context-api) — the state (or store) is attached to the component tree with `setContext`, and when you subscribe you retrieve it with `getContext`. We can do the same thing with our own state:
...
> [!NOTE] We're passing a function into `setContext` to keep reactivity across boundaries. Read more about it [here](/docs/svelte/#Passing-state-into-functions)
...
+++	let wordCount = (data.content.split(' ').length);
  let estimatedReadingTime = (wordCount / 250);+++
</script>
```

> [!NOTE] If your code in `onMount` and `onDestroy` has to run again after navigation you can use [afterNavigate](-navigation#afterNavigate) and [beforeNavigate](-navigation#beforeNavigate) respectively.
...
SvelteKit will load your `+page/layout(.server).js` files (and all files they import) for analysis during the build. Any code that should _not_ be executed at this stage must check that `building` from [`/environment`](-environment) is `false`:

```js
+++import { building } from '/environment';+++
import { setupMyDatabase } from '/server/database';
...
> [!NOTE] SvelteKit's built-in `` module should be preferred for environment variables.
...
You _can_ use it in serverless deployments, but it won't work as expected, since files are not copied from your project into your deployment. Instead, use the [`read`](-server#read) function from `/server` to access your files. `read` does not work inside edge deployments (this may change in future).
...
```js
// @errors: 2664
import { BYPASS_TOKEN } from '/static/private';
...
Vercel makes a set of [deployment-specific environment variables](https://vercel.com/docs/concepts/projects/environment-variables#system-environment-variables) available. Like other environment variables, these are accessible from `/static/private` and `/dynamic/private` (sometimes — more on that later), and inaccessible from their public counterparts. To access one of these variables from the client:

```js
// @errors: 2305
/// file: +layout.server.js
import { VERCEL_COMMIT_REF } from '/static/private';
...
You _can_ use it in serverless functions, but it won't work as expected, since files are not copied from your project into your deployment. Instead, use the [`read`](-server#read) function from `/server` to access your files. `read` does not work inside routes deployed as edge functions (this may change in future).
  ...
  const adapter = {
    ...
    supports: {
      read: ({ config, route }) => {
        ...
        // from `/server` in production, return `false` if it can't.
...
<script>
  import ReusableLayout from '/ReusableLayout.svelte';
  let { data, children } = ();
</script>
...
During prerendering, SvelteKit crawls your pages for links and renders each route it finds. Rendering the route invokes the `handle` function (and all other route dependencies, like `load`). If you need to exclude some code from running during this phase, check that the app is not [`building`](-environment#building) beforehand.
...
- you can generate a custom representation of the error that is safe to show to users, omitting sensitive details like messages and stack traces. The returned value, which defaults to `{ message }`, becomes the value of `.error`.
...
Unexpected errors will go through the [`handleError`](hooks#Shared-hooks-handleError) hook, where you can add your own error handling — for example, sending errors to a reporting service, or returning a custom error object which becomes `.error`.
...
> [!NOTE] You can also programmatically invoke `preloadData` from `/navigation`.
...
Inside the service worker you have access to the [`-worker` module](-worker), which provides you with the paths to all static assets, build files and prerendered pages. You're also provided with an app version string, which you can use for creating a unique cache name, and the deployment's `base` path. If your Vite config specifies `define` (used for global variable replacements), this will be applied to service workers as well as your server/client builds.
...
import { build, files, version } from '-worker';
...
This disables access to DOM typings like `HTMLElement` which are not available inside a service worker and instantiates the correct globals. The reassignment of `self` to `sw` allows you to type cast it in the process (there are a couple of ways to do this, but this is the easiest that requires no additional files). Use `sw` instead of `self` in the rest of the file. The reference to the SvelteKit types ensures that the `-worker` import has proper type definitions. If you import `/static/public` you either have to `// @ts-ignore` the import or add `/// <reference types="../.svelte-kit/ambient.d.ts" />` to the reference types.
...
The [`/static/private`](-static-private) and [`/dynamic/private`](-dynamic-private) modules can only be imported into modules that only run on the server, such as [`hooks.server.js`](hooks#Server-hooks) or [`+page.server.js`](routing#page-page.server.js).

## Server-only utilities

The [`/server`](-server) module, which contains a [`read`](-server#read) function for reading assets from the filesystem, can likewise only be imported by code that runs on the server.
...
- placing them in `/server`, e.g. `/server/secrets.js`
...
export { atlantisCoordinates } from '/server/secrets.js';
...
```
Cannot import /server/secrets.js into public-facing code:
- src/routes/+page.svelte
  - src/routes/utils.js
    - /server/secrets.js
```
...
This feature also works with dynamic imports, even interpolated ones like ``await import(`./.js`)``, with one small caveat: during development, if there are two or more dynamic imports between the public-facing code and the server-only module, the illegal import will not be detected the first time the code is loaded.
...
SvelteKit makes this possible with the [`pushState`](-navigation#pushState) and [`replaceState`](-navigation#replaceState) functions, which allow you to associate state with a history entry without navigating. For example, to implement a history-driven modal:
...
The second argument is the new page state, which can be accessed via the [page object](-state#page) as `page.state`. You can make page state type-safe by declaring an [`App.PageState`](types#PageState) interface (usually in `src/app.d.ts`).

To set page state without creating a new history entry, use `replaceState` instead of `pushState`.

> [!LEGACY]
> `page.state` from `/state` was added in SvelteKit 2.12. If you're using an earlier version or are using Svelte 4, use `.state` from `/stores` instead.
...
For this to work, you need to load the data that the `+page.svelte` expects. A convenient way to do this is to use [`preloadData`](-navigation#preloadData) inside the `click` handler of an `<a>` element. If the element (or a parent) uses [`data-sveltekit-preload-data`](link-options#data-sveltekit-preload-data), the data will have already been requested, and `preloadData` will reuse that request.

```svelte
<!--- file: src/routes/photos/+page.svelte --->
<script>
  import { preloadData, pushState, goto } from '/navigation';
  import { page } from '/state';
  import Modal from './Modal.svelte';
...
You should avoid using SvelteKit-specific modules like `/environment` in your packages unless you intend for them to only be consumable by other SvelteKit projects. E.g. rather than using `import { browser } from '/environment'` you could use `import { BROWSER } from 'esm-env'` ([see esm-env docs](https://github.com/benmccann/esm-env)). You may also wish to pass in things like the current URL or a navigation action as a prop rather than relying directly on `/state`, `/navigation`, etc. Writing your app in this more generic fashion will also make it easier to setup tools for testing, UI demos and so on.
...
/// <reference types="@sveltejs/kit" />
// ---cut---
import { afterNavigate } from '/navigation';
...
> [!NOTE] A common pattern is to return SEO-related `data` from page [`load`](load) functions, then use it (as [`page.data`](-state)) in a `<svelte:head>` in your root [layout](routing#layout).
...
While SvelteKit does not have any specific integration with [view transitions](https://developer.chrome.com/docs/web-platform/view-transitions/), you can call `document.startViewTransition` in [`onNavigate`](-navigation#onNavigate) to trigger a view transition on every client-side navigation.
...
`goto(...)` no longer accepts external URLs. To navigate to an external URL, use `window.location.href = url`. The `state` object now determines `.state` and must adhere to the `App.PageState` interface, if declared. See [shallow routing](shallow-routing) for more details.

## paths are now relative by default

In SvelteKit 1, `%sveltekit.assets%` in your `app.html` was replaced with a relative path by default (i.e. `.` or `..` or `../..` etc, depending on the path being rendered) during server-side rendering unless the [`paths.relative`](configuration#paths) config option was explicitly set to `false`. The same was true for `base` and `assets` imported from `/paths`, but only if the `paths.relative` option was explicitly set to `true`.
...
As such, SvelteKit 2 replaces `resolvePath` with a (slightly better named) function called `resolveRoute`, which is imported from `/paths` and which takes `base` into account.

```js
---import { resolvePath } from '@sveltejs/kit';
import { base } from '/paths';---
+++import { resolveRoute } from '/paths';+++
...
The `/dynamic/public` and `/dynamic/private` modules provide access to _run time_ environment variables, as opposed to the _build time_ environment variables exposed by `/static/public` and `/static/private`.

During prerendering in SvelteKit 1, they are one and the same. As such, prerendered pages that make use of 'dynamic' environment variables are really 'baking in' build time values, which is incorrect. Worse, `/dynamic/public` is populated in the browser with these stale values if the user happens to land on a prerendered page before navigating to dynamically-rendered pages.

Because of this, dynamic environment variables can no longer be read during prerendering in SvelteKit 2 — you should use the `static` modules instead. If the user lands on a prerendered page, SvelteKit will request up-to-date values for `/dynamic/public` from the server (by default from a module called `_env.js` — this can be configured with `config.kit.env.publicModule`) instead of reading them from the server-rendered HTML.
...
## SvelteKit 2.12: /stores deprecated

SvelteKit 2.12 introduced `/state` based on the [Svelte 5 runes API](/docs/svelte/what-are-runes). `/state` provides everything that `/stores` provides but with more flexibility as to where and how you use it. Most importantly, the `page` object is now fine-grained, e.g. updates to `page.state` will not invalidate `page.data` and vice-versa.

As a consequence, `/stores` is deprecated and subject to be removed in SvelteKit 3. We recommend [upgrading to Svelte 5](/docs/svelte/v5-migration-guide), if you haven't already, and then migrate away from `/stores`. Most of the replacements should be pretty simple: Replace the `/stores` import with `/state` and remove the `$` prefixes from the usage sites.
...
Use `npx sv migrate app-state` to auto-migrate most of your `/stores` usages inside `.svelte` components.
...
Most imports from `@sapper/service-worker` have equivalents in [`-worker`](-worker):
...
The `goto`, `prefetch` and `prefetchRoutes` imports from `@sapper/app` should be replaced with `goto`, `preloadData` and `preloadCode` imports respectively from [`/navigation`](-navigation).

The `stores` import from `@sapper/app` should be replaced — see the [Stores](migrating#Pages-and-layouts-Stores) section below.

Any files you previously imported from directories in `src/node_modules` will need to be replaced with [``]() imports.
...
You access them differently in SvelteKit. `stores` is now `getStores`, but in most cases it is unnecessary since you can import `navigating`, and `page` directly from [`/stores`](-stores). If you're on Svelte 5 and SvelteKit 2.12 or higher, consider using [`/state`](-state) instead.
...
Previously, layout components received a `segment` prop indicating the child segment. This has been removed; you should use the more flexible `.url.pathname` (or `page.url.pathname`) value to derive the segment you're interested in.
...
Test support for `read` from `/server`
...
The argument passed to [`afterNavigate`](/docs/kit/-navigation#afterNavigate) callbacks.
...
The argument passed to [`beforeNavigate`](/docs/kit/-navigation#beforeNavigate) callbacks.
...
Generate a module exposing build-time environment variables as `/dynamic/public`.
...
The generic form of `PageLoad` and `LayoutLoad`. You should import those from `./` (see [generated types](/docs/kit/types#Generated-types))
...
The generic form of `PageLoadEvent` and `LayoutLoadEvent`. You should import those from `./` (see [generated types](/docs/kit/types#Generated-types))
...
```dts
depends: (...deps: Array<`:`>) => void;
```

<div class="ts-block-property-details">

This function declares that the `load` function has a _dependency_ on one or more URLs or custom identifiers, which can subsequently be used with [`invalidate()`](/docs/kit/-navigation#invalidate) to cause `load` to rerun.
...
/// file: src/routes/+page.svelte
<script>
  import { invalidate } from '/navigation';
...
The argument passed to [`onNavigate`](/docs/kit/-navigation#onNavigate) callbacks.
...
The shape of the [`page`](/docs/kit/-state#page) reactive object and the [``](/docs/kit/-stores) store.
...
The page state, which can be manipulated using the [`pushState`](/docs/kit/-navigation#pushState) and [`replaceState`](/docs/kit/-navigation#replaceState) functions from `/navigation`.
...
A function that turns an asset filename into a `ReadableStream`. Required for the `read` export from `/server` to work
...
The generic form of `PageServerLoad` and `LayoutServerLoad`. You should import those from `./` (see [generated types](/docs/kit/types#Generated-types))
...
namespace Csp {
  ...
  type FrameSource =
    ...
    | 'self'
    | 'none';
  type HostNameScheme = `.` | 'localhost';
  type HostSource =
    ``;
  type HostProtocolSchemes = `://` | '';
  type HttpDelineator = '/' | '?' | '#' | '\';
  type PortScheme = `:` | '' | ':*';
  type SchemeSource =
    | 'http:'
...
```js
// @noErrors
import { browser, building, dev, version } from '/environment';
```
...
```js
// @noErrors
import { applyAction, deserialize, enhance } from '/forms';
```
...
```js
// @noErrors
import { assets, base, resolveRoute } from '/paths';
```
...
SvelteKit makes three read-only state objects available via the `/state` module — `page`, `navigating` and `updated`.

> [!NOTE]
> This module was added in 2.12. If you're using an earlier version of SvelteKit, use [`/stores`](-stores) instead.



```js
// @noErrors
import { navigating, page, updated } from '/state';
```
...
- retrieving the page state that was set through `goto`, `pushState` or `replaceState` (also see [goto](/docs/kit/-navigation#goto) and [shallow routing](/docs/kit/shallow-routing))
...
<script>
  import { page } from '/state';
  const id = (page.params.id); // This will correctly update id for usage on this page
...
This module contains store-based equivalents of the exports from [`/state`](-state). If you're using SvelteKit 2.12 or later, use that module instead.



```js
// @noErrors
import { getStores, navigating, page, updated } from '/stores';
```
...
Use `navigating` from `/state` instead (requires Svelte 5, [see docs for more info](/docs/kit/migrating-to-sveltekit-2#SvelteKit-2.12:--stores-deprecated))
...
Use `page` from `/state` instead (requires Svelte 5, [see docs for more info](/docs/kit/migrating-to-sveltekit-2#SvelteKit-2.12:--stores-deprecated))
...
Use `updated` from `/state` instead (requires Svelte 5, [see docs for more info](/docs/kit/migrating-to-sveltekit-2#SvelteKit-2.12:--stores-deprecated))
...
> In `dev`, `/dynamic` always includes environment variables from `.env`. In `prod`, this behavior will depend on your adapter.

# /dynamic/public

Similar to [`/dynamic/private`](/docs/kit/-dynamic-private), but only includes variables that begin with [`config.kit.env.publicPrefix`](/docs/kit/configuration#env) (which defaults to `PUBLIC_`), and can therefore safely be exposed to client-side code.

Note that public dynamic environment variables must all be sent from the server to the client, causing larger network requests — when possible, use `/static/public` instead.

Dynamic environment variables cannot be used during prerendering.

```ts
import { env } from '/dynamic/public';
console.log(env.PUBLIC_DEPLOYMENT_SPECIFIC_VARIABLE);
```

# /static/private

Environment variables [loaded by Vite](https://vitejs.dev/guide/env-and-mode.html#env-files) from `.env` files and `process.env`. Like [`/dynamic/private`](/docs/kit/-dynamic-private), this module cannot be imported into client-side code. This module only includes variables that _do not_ begin with [`config.kit.env.publicPrefix`](/docs/kit/configuration#env) _and do_ start with [`config.kit.env.privatePrefix`](/docs/kit/configuration#env) (if configured).

_Unlike_ [`/dynamic/private`](/docs/kit/-dynamic-private), the values exported from this module are statically injected into your bundle at build time, enabling optimisations like dead code elimination.

```ts
import { API_KEY } from '/static/private';
```
...
Similar to [`/static/private`](/docs/kit/-static-private), except that it only includes environment variables that begin with [`config.kit.env.publicPrefix`](/docs/kit/configuration#env) (which defaults to `PUBLIC_`), and can therefore safely be exposed to client-side code.

Values are replaced statically at build time.

```ts
import { PUBLIC_BASE_URL } from '/static/public';
```

# 

SvelteKit automatically makes files under `src/lib` available using the `` import alias. You can change which directory this alias points to in your [config file](configuration#files).
...
```js
// @noErrors
import { base, build, files, prerendered, version } from '-worker';
```
...
> [!NOTE] The built-in `` alias is controlled by `config.kit.files.lib` as it is used for packaging.
...
If `paths.assets` is specified, there will be two app directories — `${paths.assets}/` and `${paths.base}/`.
...
A prefix that signals that an environment variable is safe to expose to client-side code. See [`/static/public`](/docs/kit/-static-public) and [`/dynamic/public`](/docs/kit/-dynamic-public). Note that Vite's [`envPrefix`](https://vitejs.dev/config/shared-options.html#envprefix) must be set separately if you are using Vite's environment variable handling - though use of that feature should generally be unnecessary.
...
A prefix that signals that an environment variable is unsafe to expose to client-side code. Environment variables matching neither the public nor the private prefix will be discarded completely. See [`/static/private`](/docs/kit/-static-private) and [`/dynamic/private`](/docs/kit/-dynamic-private).
...
your app's internal library, accessible throughout the codebase as ``
...
A root-relative path that must start, but not end with `/` (e.g. `/base-path`), unless it is the empty string. This specifies where your app is served from and allows the app to live on a non-root path. Note that you need to prepend all your root-relative links with the base value or they will point to the root of your domain, not your `base` (this is how the browser works). You can use [`base` from `/paths`](/docs/kit/-paths#base) for that: `<a href="{base}/your-page">Link</a>`. If you find yourself writing this often, it may make sense to extract this into a reusable component.
...
If `true`, `base` and `assets` imported from `/paths` will be replaced with relative asset paths during server-side rendering, resulting in more portable HTML.
...
In 1.0, `undefined` was a valid value, which was set by default. In that case, if `paths.assets` was not external, SvelteKit would replace `%sveltekit.assets%` with a relative path and use relative paths to reference build artifacts, but `base` and `assets` imported from `/paths` would be as specified in your config.
...
Determine which files in your `static` directory will be available in `-worker.files`.
...
/// file: +layout.svelte
<script>
  import { beforeNavigate } from '/navigation';
  import { updated } from '/state';
...
If you set `pollInterval` to a non-zero value, SvelteKit will poll for new versions in the background and set the value of [`updated.current`](/docs/kit/-state#updated) `true` when it detects one.
...
`svelte-kit sync` creates the `tsconfig.json` and all generated types (which you can import as `./` inside routing files) for your project. When you create a new project, it is listed as the `prepare` script and will be run automatically as part of the npm lifecycle, so you should not ordinarily have to run this command.
...
The return types of the load functions are then available through the `` module as `PageData` and `LayoutData` respectively, while the union of the return values of all `Actions` is available as `ActionData`. Starting with version 2.16.0, two additional helper types are provided. `PageProps` defines `data: PageData`, as well as `form: ActionData`, when there are actions defined. `LayoutProps` defines `data: LayoutData`, as well as `children: Snippet`:
...
A subdirectory of ``. SvelteKit will prevent you from importing any modules in `/server` into client-side code. See [server-only modules](server-only-modules).
...
Defines the common shape of the [page.data state](/docs/kit/-state#page) and [.data store](/docs/kit/-stores#page) - that is, the data that is shared between all pages.
The `Load` and `ServerLoad` functions in `./` will be narrowed accordingly.
...
The shape of the `page.state` object, which can be manipulated using the [`pushState`](/docs/kit/-navigation#pushState) and [`replaceState`](/docs/kit/-navigation#replaceState) functions from `/navigation`.
